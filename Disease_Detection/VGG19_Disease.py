...
class VGG19_Disease:
  ...
  def Disease_Architecture(self,Train_data_preprocess,Test_data_preprocess):
    try:
      ...
      x= Flatten()(vgg19.output) # 1d array
      predict=Dense(3,activation='softmax')(x) # two or more classes
      model=Model(inputs=vgg19.inputs,outputs=predict) # taking vgg16 inputs
      print(model.summary())
  ...
  def Disease_prediction(self,model):
    try:
        image=cv2.imread('F:/computer vision/code/er/Disease_detection/disease/train/COVID19/COVID19(217).jpg')
